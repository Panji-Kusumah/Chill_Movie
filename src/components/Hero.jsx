        ...
        <section className="relative h-[60vh] md:h-[95vh] w-full overflow-hidden bg-[#181818]">
            ...
            <div className="absolute bottom-[20%] left-[20px] md:left-[80px] z-20 w-[90%] md:w-[50%] flex flex-col gap-3 md:gap-5">
                ...
                <div className="flex flex-wrap items-center gap-3 md:gap-4 mt-2">
                    <button
                        onClick={togglePlay}
                        className="bg-[#2F80ED] hover:bg-[#2F80ED]/80 text-white px-6 py-1.5 md:px-8 md:py-2 rounded-full flex items-center gap-2 transition duration-300 shadow-[0_0_15px_rgba(47,128,237,0.4)] font-bold text-[14px] md:text-base"
                    >
                        <img src={playIcon} alt="Play" className="w-4 h-4 md:w-5 md:h-5" />
                        {isPlaying ? "Pause" : "Mulai"}
                    </button>
                    <button className="bg-[#2F3334]/80 hover:bg-[#2F3334] text-white px-4 py-1.5 md:px-6 md:py-2 rounded-full flex items-center gap-2 transition duration-300 font-semibold backdrop-blur-md text-[14px] md:text-base">
                        <img src={infoIcon} alt="Info" className="w-4 h-4 md:w-5 md:h-5 invert" />
                        Selengkapnya
                    </button>
                    <div className="flex items-center gap-2 md:gap-3">
                        <span className="border border-white/60 text-white flex items-center justify-center w-[35px] h-[35px] md:w-[40px] md:h-[40px] rounded-full text-[12px] md:text-[14px] font-bold bg-black/20">
                            18+
                        </span>
                        <button
                            onClick={toggleMute}
