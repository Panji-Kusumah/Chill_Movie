...
// Import Store Autentikasi --- tandai biar ga lupa 
...
const Navbar = () => {
    ...
    // ngambil user aktif dan fungsi logout
    ...
    return (
        <nav className="fixed top-0 left-0 w-full z-50 bg-[#181818]/80 backdrop-blur-md px-[20px] md:px-[80px] py-4 flex justify-between items-center transition-all">
            <div className="flex items-center gap-[20px] md:gap-[40px]">
                <Link to="/home">
                    <img src={logo} alt="Chill" className="h-[30px] md:h-[40px] w-auto" />
                </Link>
                <div className="hidden md:flex items-center gap-[30px] text-white font-semibold opacity-80">
            ...
            <div className="relative" ref={dropdownRef}>
                <div onClick={() => setOpen(!open)} className="flex items-center gap-2 cursor-pointer group">
                    <img src={avatar} alt="Avatar" className="w-[32px] md:w-[40px] h-[32px] md:h-[40px] rounded-full border-2 border-transparent group-hover:border-[#2F80ED] transition-all" />
                    {/* nampilin nama di avatar */}
